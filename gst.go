...
func CreatePipeline(codecName string, videoPassthrough bool, bitrate string, tracks []*webrtc.TrackLocalStaticSample, pipelineSrc string) *Pipeline {
	...
	switch codecName {
	case "vp8":
		pipelineStr = pipelineSrc + " ! vp8enc " + "target-bitrate=" + bitrate + " error-resilient=partitions keyframe-max-dist=10 auto-alt-ref=true cpu-used=5 deadline=1 ! " + pipelineStr
